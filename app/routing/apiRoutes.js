...
module.exports = function(app) {
  ...
  //A GET route with the url /api/voters/:candidate. This will be used to display a JSON of all voters matched with the possible candidate.
  app.get("/api/voters/:candidate", function(req, res) {
    var similarVoters = [];

    for (i = 0; i < voters.length; i++) {
      var candidate = voters[i].matchedPolitician
        .replace(/\s/g, "")
        .toLowerCase();
